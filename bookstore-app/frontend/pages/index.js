...
export default function HomePage() {
  const { books, loading, error, fetchBooks, getBestsellers } = useBookStore();
  ...
  }, [activeTab, authors.length, genres.length]);
  ...
  const fetchAuthors = async () => {
    ...
    try {
      ...
      setAuthorsError(error.response?.data?.error || error.message || 'Failed to fetch authors');
    } finally {
  ...
  const fetchGenres = async () => {
    ...
    try {
      ...
      setGenresError(error.response?.data?.error || error.message || 'Failed to fetch genres');
    } finally {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'authors' && (
          <section>
            ...
            {authorsError && (
              <div className="text-center py-12">
                <p className="text-red-600">Error: {authorsError}</p>
                <button 
        ...
        {activeTab === 'genres' && (
          <section>
            ...
            {genresError && (
              <div className="text-center py-12">
                <p className="text-red-600">Error: {genresError}</p>
                <button 
                  onClick={() => fetchGenres()} 
                  className="mt-2 px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700"
                >
